document.getElementById('editAddressForm').addEventListener('submit', async (e) => {
  ...
  if (!state) { showFieldError('state', 'Please select a state'); valid = false; }
  if (!city) { showFieldError('city', 'City is required'); valid = false; }
  if (!fullAddress) { showFieldError('fullAddress', 'Full address is required'); valid = false; }
  if (!label) { showFieldError('label', 'Please select an address type'); valid = false; }
